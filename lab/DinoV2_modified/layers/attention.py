...
class MemEffAttention(Attention):
    def forward(self, x: Tensor, g_info: Tensor = None, attn_bias=None) -> Tensor:
        ...
        if True:
            ...
            #B,N,head
            #B,N,1,1
            # generate new tensor of shape (B, N, 1, 1), where first 30% of N is Ture and rest is False
            q_bool = torch.zeroes(B, N).to(q_pure.device)
            q_bool = q_bool[:, :int(0.3 * N)] = 1
            

            q_sim_pos = q_bool > 0  # we have ablated this from 0.6 to 0.9
            q_sim_neg = not q_sim_pos
